...
def date_search(options):
    ...
    for start, end in date_ranges:
        tasks.fetch_date_range.apply_async(
            kwargs={"start": start, "end": end}, queue=options["queue"]
        )
...
def add_or_update_case(options):
    ...
    if options["case"] is None:
        print(
            "--case is a required parameter when the add-case action is "
            "requested."
        )
    else:
...
def process_case_queue(options):
    ...
    for case_id in case_ids:
        throttle.maybe_wait()
        tasks.add_or_update_case_db.apply_async(
            kwargs={"case_id": case_id}, queue=q
        )
